...
from app.models import ShoppingContext
...
class LocalJSONContextStore:
    """
    File-based JSON storage for ShoppingContext.
    """
    def __init__(self, file_path: str = "shopping_context.json"):
        self.file_path = file_path
        self._context: ShoppingContext = self._load()

    def _load(self) -> ShoppingContext:
        if not os.path.exists(self.file_path):
    ...
    def save(self, context: ShoppingContext) -> None:
        try:
    ...
    def get_context(self) -> ShoppingContext:
        return self._context

    def clear(self) -> None:
        logger.info("Clearing shopping context")
        self._context = ShoppingContext()
