...
class ChatService:
    ...
    async def generate_chat_completion(self, chat_request: ChatMessageRequest) -> ChatMessageResponse:
        ...
        if chat_exists:
            ...
            existing_chat = await self._chat_repository.get_chat_by_id(chat_request.chat_id)
            ...
            await self._chat_repository.create_chat(new_chat)
    ...
    async def list_chats(self) -> list[Chat]:
        """Retrieve all chats."""
        return await self._chat_repository.list_all_chats()

    async def delete_chat(self, chat_id: ULID) -> None:
        """Delete a chat by its ID."""
        await self._chat_repository.delete_chat_by_id(chat_id)
